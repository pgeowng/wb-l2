...
// Паттерн помогает:
//   + Обобщить создание объектов, так же как интерфейсы обобщают поведение.
//     Например, для Windows/Mac/Web версии использовать разные фреймворки, с одинаковым интерфейсом.
//   + Уменьшить связь между созданием и поведением класса.
//     Например, используем Factory для темной и светлой темы UI.
//     При этом поведение никак не затронуто.

// - Возрастает общая сложность кода.
...
// Concrete Product
// К нам могут загрузить как изображение, так и видео.
// Мы же хотим подготовить превью контента на основе размеров и вида.
// Каждый вид реализует свой способ конвертации.
...
// Concrete Creator
// Например, у клиента медленный интернет.
// Мы подготавливаем минимальный по размеру файл и отдаем его.
